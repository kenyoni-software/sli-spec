...
The fully qualified SLI **MUST** be unique within a project.
SLIs are stable identifiers. They **MUST** remain unchanged in any released, distributed or otherwise published version of the project, as changing them breaks the link between code and translations. Translation content **MAY** be updated freely at any time. During development prior to release, SLIs **MAY** be changed freely.
...
`KEY_REGEX` is a placeholder representing the regular expression that validates the key, including placeholders.
...
Backus–Naur Form (BNF) grammar for SLIs with single curly braces `{}` as placeholder delimiters:
...
Each example line below represents a separate project.
...
### 6.8 Invalid / disallowed examples
...
- `ui/button.cancel{size-number}`: `Cancel` — Hyphen used inside placeholder-like token, placeholder names **MUST** contain only `a–z`, `0–9`, `_`.
...
- `@ui/button.cancel` and `ui/buttons.cancel` — Do not mix prefixed and unprefixed SLIs within the same project.
- `@ui/button.cancel_{username}` and `ui/buttons.ok_{{username}}` — Do not mix different placeholder delimiter syntaxes within the same project.
